# step34b_v30b.py - v3.0b: パイロット故障修正版 (step34の後継。step34は使用停止)
...
def support_idxs(v, excl):
    return [i for i, qt in enumerate(qtexts) if v in qt and i not in excl]
...
OUT = f"results_v30b_{judge.pv}_t{args.theta}_fb{args.tau_fb}.jsonl"
done = set()
...
for p in parsed:
    ...
    for cl in p["parsed"]["clauses"]:
        ...
        dict_members |= {d for d in cand
                         if min(f.get(d, 0) for f in fms) >= args.theta}
    ...
    for cl in unresolved:
        ...
        for d in sorted(cands - dict_members):
            s = score(p["idx"], q, d)
            if s is not None and s >= args.tau_fb:
                scored.append((s, d))
        scored.sort(reverse=True)
        judge_members |= {d for _, d in scored[:args.fb_cap]}
    ...
    rec = {"idx": p["idx"], "task": task, "answer_f1": af, "member_f1": mf,
           ...
           "n_fill": len(fill_members), "n_unresolved": len(unresolved),
    ...
    print(f"[idx {p['idx']:>5}] {task:13s} aF={af:.2f} mF={mf:.2f} "
          ...
          f"/fill{len(fill_members)}) 未解決句{len(unresolved)} 累計コール{n_calls}",
          flush=True)

res = [json.loads(l) for l in open(OUT, encoding="utf-8") if l.strip()]
by = collections.defaultdict(list)
for r in res: by[r["task"]].append(r)
print(f"\n=== v3.0b ({judge.pv}, θ={args.theta}, τ_fb={args.tau_fb}, "
...
print("参考: 辞書のみ32.9 / v2.5=8.85 / MARAG-7B=28.60, 14B=31.22")
