# step34c_v30c.py - v3.0c: パイロット2の故障修正版 (step34bの後継)
...
OUT = f"results_v30c_{judge.pv}_t{args.theta}_fb{args.tau_fb}.jsonl"
done = set()
...
for p in parsed:
    ...
    for cl in p["parsed"]["clauses"]:
        ...
        if not mm:
            unresolved.append(cl)   # 解決したが空 -> judgeフォールバックへ回す
            continue
        dict_members |= mm
    ...
    # 絶対に空で答えない: member空なら採点上位rescue_top件を採用 (τ_fb不問)
    rescued = set()
    if not member and all_scored:
...
print(f"\n=== v3.0c ({judge.pv}, θ={args.theta}, τ_fb={args.tau_fb}, "
