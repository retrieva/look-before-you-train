# step34g_shard.py - v3.0f凍結版のシャード並列ラッパ (ロジックはstep34fと完全同一)
#   変更は並列運用のみ: --shard i/k で idx%k==i の問だけ処理。
#   キャッシュはシャード専用ファイルに書き (同時書込衝突の回避)、既存の
#   calib_cache.jsonl と全 calib_cache_shard*.jsonl を読み取り専用で共有。
#   結果もシャード別ファイル。全シャード完了後の統合:
#     cat results_v30f_test_shard*.jsonl > results_v30f_test_all.jsonl
#     python step34g_shard.py --parsed parsed2_test.jsonl --aggregate results_v30f_test_all.jsonl
#   使い方 (3ターミナル同時):
#     python step34g_shard.py --parsed parsed2_test.jsonl --n 99999 --shard 0/3 --workers 12
#     python step34g_shard.py --parsed parsed2_test.jsonl --n 99999 --shard 1/3 --workers 12
#     python step34g_shard.py --parsed parsed2_test.jsonl --n 99999 --shard 2/3 --workers 12
# 元ヘッダ: step34f_v30f.py - v3.0f (凍結)
#   train60本測 (v30d/e): ALL 39.6 (P2d>=48 失敗)。分割集計の裁定:
#     辞書のみ群 aF 69.7 (count除外87.4) / judge関与群 19.0 / 救済 20.0
#   反実仮想: judgeが助けた問は辞書0件の問だけ (1078: 0->1.00, 10863: 0->0.50)。
#   辞書が部分解決した問ではjudgeは壊す (6889: 0.92->0.37)。
#   count較正: 保留126問 (OR型, train60外) でθ掃引 -> θ_count=0.45が完全一致62.5%。
#   v30f変更 (2点のみ):
#     1. judge層は辞書memberが完全に空のときだけ起動 (部分解決問はjudge禁止)
#     2. countのみ member閾値 θ_count=0.45 (保留セット較正値, --theta-count)
#
# 事前予測 (2026-07-07 登録):
#   P1f: train60 ALL >= 43 (judge破壊の除去でjudge関与群が辞書のみ水準へ回復)
#   P2f: 新規コール < 200 (ほぼキャッシュ命中)
#   P3f: count aFは±7pt内 (θ_countはOR型にしか効かない, train60は複雑型が多い)
#   採点はThreadPoolExecutorで並列化 (--workers, 既定8)。キャッシュ書き込みはロック保護。
#   25コールごとにハートビート表示 (無言区間の解消)。
#   member構成ロジックはv30dと完全同一 -> 結果ファイルもv30dを共有 (続きから再開可能)
# 元: step34d_v30d.py - v3.0d: 空クローズ規則の修正 (step34cの後継)
...
import json, os, re, random, argparse, collections, threading
...
ap.add_argument("--workers", type=int, default=8, help="採点の並列数")
ap.add_argument("--theta-count", type=float, default=0.45, dest="theta_count",
                help="count専用member閾値 (保留126問較正: 完全一致62.5%%)")
ap.add_argument("--shard", default="0/1", help="i/k: idx%%k==iの問のみ処理")
ap.add_argument("--aggregate", default=None,
                help="統合resultsファイルを集計だけして終了 (課金ゼロ)")
...
if args.aggregate:
    ...
    res=[json.loads(l) for l in open(args.aggregate, encoding="utf-8") if l.strip()]
    seen=set(); uniq=[]
    for r in res:
        if r["idx"] not in seen:
            seen.add(r["idx"]); uniq.append(r)
    by=_c.defaultdict(list)
    for r in uniq: by[r["task"]].append(r)
    print(f"=== 統合集計 ({args.aggregate}, n={len(uniq)}) ===")
    import numpy as _np
    for t,rs in sorted(by.items()):
        print(f"{t:>13} | {len(rs):>4} | mF={_np.mean([r['member_f1'] for r in rs])*100:5.1f}% | aF={_np.mean([r['answer_f1'] for r in rs])*100:5.1f}%")
    print(f"{'ALL':>13} | {len(uniq):>4} | mF={_np.mean([r['member_f1'] for r in uniq])*100:5.1f}% | aF={_np.mean([r['answer_f1'] for r in uniq])*100:5.1f}%")
    raise SystemExit

SHARD_I, SHARD_K = (int(x) for x in args.shard.split("/"))
...
import glob
cache = {}
for cpath in ["calib_cache.jsonl"] + sorted(glob.glob("calib_cache_shard*.jsonl")):
...
CACHE = f"calib_cache_shard{SHARD_I}.jsonl"
cf = open(CACHE, "a", encoding="utf-8")
n_calls = 0
_lock = threading.Lock()

def _score_one(idx, question, d):
    global n_calls
    s = judge.call(question, raw_text[d][:24000])
    with _lock:
        n_calls += 1
        cache[(judge.pv, idx, d)] = s
        cf.write(json.dumps({"pv": judge.pv, "idx": idx, "d": d, "s": s},
                            ensure_ascii=False) + "\n")
        cf.flush()
        if n_calls % 25 == 0:
            print(f"    ...採点中 累計{n_calls}コール", flush=True)
    return s

def score_many(idx, question, ds):
    """キャッシュ命中分は即返し、未採点分のみ並列実行。[(score, d)]を返す"""
    out = []
    todo = []
    for d in ds:
        k = (judge.pv, idx, d)
        if k in cache:
            out.append((cache[k], d))
        elif not args.no_judge:
            todo.append(d)
    if todo:
        with ThreadPoolExecutor(max_workers=args.workers) as ex:
            for d, s in zip(todo, ex.map(lambda d: _score_one(idx, question, d), todo)):
                out.append((s, d))
    return out

def score(idx, question, d):
    r = score_many(idx, question, [d])
...
parsed = [p for p in parsed if p["idx"] % SHARD_K == SHARD_I]
print(f"シャード {SHARD_I}/{SHARD_K}: {len(parsed)}問を担当")
OUT = f"results_v30f_test_shard{SHARD_I}.jsonl"
done = set()
...
for p in parsed:
    ...
    # 第一層: 辞書 (countのみθ_count)
    theta_q = args.theta_count if task == "count" else args.theta
    ...
    for cl in p["parsed"]["clauses"]:
        ...
        mm = {d for d in cand if min(f.get(d, 0) for f in fms) >= theta_q}
        # 解決済みだが空 -> 空は情報。何も足さずjudgeにも回さない (v30d)
        dict_members |= mm

    # v30f: 辞書が1件でも出したらjudge層は起動しない (部分解決問の破壊防止)
    if dict_members:
        unresolved = []
    ...
    for cl in unresolved:
        ...
        scored = score_many(p["idx"], q, sorted(cands - dict_members))
        ...
        if not all_scored:
            ...
            for cl in p["parsed"]["clauses"]:
                for ph in cl:
                    ...
                    if ph in bge:
                        cands |= set(bge[ph][:args.m])
            all_scored.extend(score_many(p["idx"], q, sorted(cands)))
        if all_scored:
    ...
    if args.fill and judge_members:
        ...
        for _ in range(args.fill_iters):
            ...
            new = {d for s, d in score_many(p["idx"], q, sorted(neigh)) if s >= thr}
...
print(f"\n=== v3.0f ({judge.pv}, θ={args.theta}, τ_fb={args.tau_fb}, "
